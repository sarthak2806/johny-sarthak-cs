...
try:
    ...
    Button(master,text='Enter',command=show).grid(row=3, column=1, sticky=W, pady=4)
except:
    pass
...
cur=sql.cursor()
try:
    cur.execute('create table score (Name char(100),Score int(2))')
except:
    pass
com='insert into score (Name,Score) values(%s,%s)'
val=(str(first+' '+last),str(counter))
cur.execute(com,val)
sql.commit()